        ...
        try {
            ...
            for (const anime of animeData) {
                ...
                if (!existingAnime) {
                    ...
                    console.log(`Anime ${title} (ID: ${id}) already exists. Updating episodes...`);
...
async function fetchStreamLinks(episodeId) {
    try {
        const rawStream = await fetchWithRetry(`https://vimal.animoon.me/api/stream?id=${episodeId}&server=hd-2&type=raw`);
        const subStream = await fetchWithRetry(`https://vimal.animoon.me/api/stream?id=${episodeId}&server=hd-2&type=sub`);
        const dubStream = await fetchWithRetry(`https://vimal.animoon.me/api/stream?id=${episodeId}&server=hd-2&type=dub`);
...
async function updateAnimeEpisodes(animeCollection, animeId, newEpisodes) {
    try {
        ...
        console.error(`Error updating episodes for anime ID ${animeId}: ${error.message}`);
    }
}
