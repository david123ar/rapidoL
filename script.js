...
async function fetchAndUpdateData() {
  ...
  try {
    ...
    for (const category of categories) {
      ...
      do {
        const apiUrl = `https://vimal.animoon.me/api/${category}?page=${page}`;
        const response = await fetch(apiUrl);

        if (!response.ok) {
          console.error(
            `Failed to fetch category "${category}" page ${page}: ${response.statusText}`
          );
          break;
        }
        ...
        if (!data.success || !data.results || !data.results.data) {
          console.error(
            `Invalid or empty response for category "${category}" page ${page}`
          );
          break;
        }
